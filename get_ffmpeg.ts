...
export async function get_ffmpeg(nw_path?: string) {
  return new Promise(async function promise(resolve) {
    const package_json = JSON.parse(String(await fs.promises.readFile("package.json")));
    ...
    const temp_folder = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ffmpeg-"));
      ...
      zip.on("entry", function (entry) {
        if (/\/$/.test(entry.fileName)) {
          ...
          zip.openReadStream(entry, function (err, read_stream) {
            ...
            read_stream.pipe(
              fs.createWriteStream(path.join(destination_path, entry.fileName)).on("close", resolve),
            );
          });
        }
      });
